...
const Player3=()=>{
	...
	const url = 'ws://'+store.data.host+':'+store.data.port+'/api/v1/h5srtcapi?token='+store.data.video4+'&profile=main&session='+store.data.session+''
	...
	return (
		...
		// 	uri: 'http://' + store.data.host + ':' + store.data.port + '/api/v1/GetImage?token='+store.data.video4+'&session=' + store.data.session+' '}} resizeMode='contain' style={{width: '100%', height: '100%'}} />: <RTCView objectFit='cover' streamURL={remoteStream?.toURL()} style={styles.stream} onFirstFrame={() => {console.log('视频第一');
		// 	}}/>	}
		// </View>
	);
};
